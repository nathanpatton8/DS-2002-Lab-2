# -*- coding: utf-8 -*-
"""
Created on Fri Sep 23 15:20:13 2022
...
#%%Using cursor to run SQL queries
...
#%%Creating a tuple that contains information about the user
user = ('00002', 'Lois', 'Lane', 'Female')

#%%Adding the tuple to the database
cur.execute("INSERT INTO users VALUES(?, ?, ?, ?);", user)
conn.commit()

#%%Creating a list of tuples that contains info about the user
more_users = [('00003', 'Peter', 'Parker', 'Male'), ('00004', 'Bruce', 'Wayne', 'male')]

#%%Adding more users to the table from a list of tuples
...
#%%Fetching one result using fetchone()
cur.execute("SELECT * FROM users;")
one_result = cur.fetchone()
print(one_result)

#%%Fethcing many results using fetchmany()
cur.execute("SELECT * FROM users;")
three_results = cur.fetchmany(3)
print(three_results)

#%%Using fetchall() to fetch all the results
cur.execute("SELECT * FROM users;")
all_results = cur.fetchall()
print(all_results)

#%%Deleting users with the last name Parker
cur.execute("DELETE FROM users WHERE lname='Parker';")
conn.commit()
cur.execute("select * from users where lname='Parker'")
print(cur.fetchall())

#%%Using joins the generate a query that includes the first and last name for each customer on each order
